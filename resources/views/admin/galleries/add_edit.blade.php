...
<div class="row row-eq-height my-3">
        ...
        <form action="{{$action_url}}" method="POST" enctype="multipart/form-data" class="needs-validation @if($errors->any()) was-validated @endif" novalidate>
            ...
            <div class="form-row">
                ...
                <div class="form-group col-8 mb-2">
                    ...
                    <input type="text" class="form-control" name="title" value="{{$gallery?$gallery->title:old('title')}}" required>
                    @error('title')
    ...
    <x-panel-card class="col-md-12 mt-4 mb-5" >
        ...
        <form id="image_sequence_form" method="POST" action="{{route('galleries.image.update.sequence',['id'=>$gallery->id])}}">
            @csrf
            @if( count($gallery->images_by_sequence)>0 )
            ...
            <table class="table cursor-pointer table-bordered table-hover" style="width:100%;">
                ...
                <tbody class="sortable">
                    @forelse ($gallery->images_by_sequence as $key => $image)
                        <tr class="image_row">
                            <td>
                            ...
                            <td width="50%">
                                @if ($image->source)
                                    @if( ($image->type == App\Models\GalleryImage::TYPE_IMAGE) )
                                        <img style="height:100px" class="border" src="{{ $image->image_url }}">
                                    @elseif( $event_image['type'] == App\Models\GalleryImage::TYPE_VIDEO )
    ...
    $(document).on('click', productImageRemoveBtnClass, function() {
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                // alert(image_id);
                $.ajax({
                    url: "{{route('galleries.image.delete')}}",
                    type: 'POST',
                    data: {
                        "image_id": image_id,
                        "type": 'gallery',
